...
class TubeDETR(nn.Module):
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            # action_query_embed = self.action_query_embed.weight.unsqueeze(1)
            # query_embed_new = torch.cat((memory_cache["query_embed"], action_query_embed), 0) #129*1*256
            # action_query_mask = self.action_query_mask.to(memory_cache["query_mask"].device)
            # query_mask_new = torch.cat((memory_cache["query_mask"], action_query_mask), -1) #1*129

            # print(query_embed_new.shape)
            # print(query_mask_new.shape)
            # exit(0)
            ...
            # print(hs[-1].size())
            # print(action_hs.size())
            # print(self.action_prototype_embed.weight[:, :len(self.video_action_list)+1].size())
            ...
            if self.attribute:
                ...
                # scene_prediction = F.softmax(scene_prediction, dim=-1)
...
class SetCriterion(nn.Module):
    ...
    def loss_attribute(self, outputs, attribute_target):
        ...
        # print("attribute_target11111111111111111111111111111111111111")
        # print(sentiment_prediction_list.size())
        # print(len(sentiment_target_list))
        # print(len(sentiment_target_list[0]))
        # print(attribute_target['video_original_id'])
        # if len(sentiment_target_list[0])!=sentiment_prediction_list.size()[1]:
        ...
        for index in range(len(sentiment_target_list[0])):
            # if index == 162:
            #     print(attribute_target['video_original_id'])
            # sentiment_target = torch.cat([sentiment_target for sentiment_target in sentiment_target_list], dim=0)
        ...
        for index in range(len(scene_target_list[0])):
            ...
            # scene_target = torch.cat([scene_target for scene_target in scene_target_list], dim=0)
...
def build(args):
    ...
    weight_dict = {
        ...
        "action": 0,#0.000001,#0.04,#0.01,#0.05, #0.03, #0.04
        "attribute": 0#0.0001#1.0
    }
    if args.guided_attn:
